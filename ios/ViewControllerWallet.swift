...
class ViewControllerWallet: UIViewController {
    ...
    private func cardInformation() -> Card {
        return Card(panTokenSuffix: "3515", holder: cardholderName ?? "Unknown", cardType: paymentNetwork ?? .masterCard)
    }

    public func configure(cardholderName: String, primaryAccountSuffix: String, paymentNetwork: PKPaymentNetwork, networkRequest: NetworkRequest) {
